...
/**
 * @ author：mo
 * @ data：2020/8/10:14:10
 * @ 功能：
 */
public abstract class BGARecyclerViewAdapter<M> extends RecyclerView.Adapter<KRecycleViewHolder> {
    ...
    public KRecycleViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        KRecycleViewHolder holder = new KRecycleViewHolder(LayoutInflater.from(mContext).inflate(viewType, parent, false));
    ...
    @Override
    public void onBindViewHolder(KRecycleViewHolder viewHolder, int position) {
        // 在设置值的过程中忽略选中状态变化
     ...
     * @param model
     */
    protected abstract void fillData(KRecycleViewHolder helper, int position, M model);
