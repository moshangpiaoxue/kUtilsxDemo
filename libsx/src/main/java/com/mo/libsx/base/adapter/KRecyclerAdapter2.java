...
public abstract class KRecyclerAdapter2<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    /** 数据观察者对象 */
    private KAdapterDataObserver2 mObserver;

    public KRecyclerAdapter2(Context context, List<T> datas) {
        setRealAdapter(new BaseRecycleViewAdapter<T>(context, datas) {
            @Override
            public void doWhat(KRecycleViewHolder holder, T bean, int position, int itemViewType, RecyclerView mRecyclerView) {
                toDo(holder, bean, position, itemViewType, mRecyclerView);
            }

            @Override
            public int getItemLayout(int viewType) {
                return getItemLayout2(viewType);
            }
        });
    }

    protected abstract int getItemLayout2(int viewType);

    protected abstract void toDo(KRecycleViewHolder holder, T bean, int position, int itemViewType, RecyclerView mRecyclerView);
    ...
    public void setRealAdapter(RecyclerView.Adapter adapter) {
        if (mRealAdapter != adapter) {
            ...
            if (mRealAdapter != null) {
                if (mObserver == null) {
                    mObserver = new KAdapterDataObserver2(this);
                }
    ...
    public void onViewRecycled(@NonNull RecyclerView.ViewHolder holder) {
                if (holder instanceof KRecycleViewHolder) {
